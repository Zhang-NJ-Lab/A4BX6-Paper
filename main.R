...
# ml
## task construction
task_homo <- TaskRegr$new(id = "task_homo", backend = homod, target = "homo")
task_lumo <- TaskRegr$new(id = "task_lumo", backend = lumod, target = "lumo")
task_bandgap <- TaskRegr$new(id = "task_bandgap", backend = bandgapd, target = "bandgap")

## filter of features =============
# write_rank(task = task_homo, wd = 'results/rank/homo/',type = 'regr')
# write_rank(task = task_lumo, wd = 'results/rank/lumo/',type = 'regr')
# write_rank(task = task_bandgap, wd = 'results/rank/bandgap/',type = 'regr')
# HOMO ========================== 
cl <- mlr_learners$keys()[str_detect(mlr_learners$keys(), 'regr')]
## train and test
# SVM ===============
learner <- mlr_learners$get("regr.svm")
# learner$param_set$values = list(kernel = 'polynomial',type = 'eps- regression', cost = 2.7, gamma = 4.8, degree = 7) 
# learner$param_set$values = list(kernel = 'polynomial',
#                                 type = 'eps-regression',
#                                 degree = 3)
learner$param_set$values = list(kernel = 'polynomial',
                                type = 'eps-regression',
                                degree = 4,
                                cost = 12.31154)
# sink(file = "results/ML_para/SVM.txt")
# learner$param_set$print()
# sink()
# train_set <- sample(1:NN, size = 0.8*NN,replace = FALSE)
# test_set <- setdiff(1:NN,train_set)
# write_csv(data.frame(train_set), "results/temp_data/train_set.csv") 
# write_csv(data.frame(test_set), "results/temp_data/test_set.csv")
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]]
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]]
learner$train(task_homo, row_ids = train_set)
prediction <- learner$predict(task_homo, row_ids = train_set) 
train_set_pic_SVM <- predplot(prediction, title = 'SVM: train set')
# ggsave(filename = "results/ML_vil/SVMtrain_set_pic.jpeg", plot = train_set_pic_SVM, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
prediction <- learner$predict(task_homo, row_ids = test_set) 
test_set_pic_SVM <- predplot(prediction, title = 'SVM: test set')
# ggsave(filename = "results/ML_vil/SVMtest_set_pic.jpeg", plot = test_set_pic_SVM, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set
learner$predict_sets <- c("test", "train")
search_space = ps(
  # cost = p_dbl(lower = 0.01, upper = 20),
  # degree = p_int(lower = 1, upper = 30)
  coef0 = p_dbl(lower = -5, upper = 20),
  cross = p_int(lower = 0, upper = 20)
  # epsilon = p_dbl(lower = 1, 5)
  )
search_space
hout = rsmp("custom") 
hout$instantiate(task_homo,
                 train = list(train_set),
                 test = list(test_set)) 
measure = msr("regr.mse")
measure$predict_sets <- c("train", "test")
evals20 = trm("evals", n_evals = 1e2) 
...
trn1 = tnr("grid_search", resolution = 40, batch_size = 10) 
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals 
# learner$train(task)
  
# kknn ===============
learner <- mlr_learners$get("regr.kknn")
learner$param_set$values = list(k = 2, distance = 4.636364, scale = T)
# sink(file = "results/ML_para/kknn.txt")
# learner$param_set$print()
# sink()
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]] 
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]]
...
train_set_pic_kknn <- predplot(prediction, title = 'knn: train set')
# ggsave(filename = "results/ML_vil/kknntrain_set_pic.jpeg", plot = train_set_pic_kknn, width = unit(5.3,"cm"), height = unit(4.3,"cm")) 
prediction <- learner$predict(task_homo, row_ids = test_set)
measure <- msr("regr.mse")
mse <- format(prediction$score(measure), digits = 5)
measure <- msr("regr.rmse")
...
tss <- sum((actual - mean(actual)) ^ 2) ## total sum of squares
rsq <- round(1 - rss/tss, 2)
test_set_pic_kknn <- predplot(prediction, title = 'knn: test set')
# ggsave(filename = "results/ML_vil/kknntest_set_pic.jpeg", plot = test_set_pic_kknn, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set 
learner$predict_sets <- c("test", "train")
search_space = ps(
  k = p_int(lower = 1, upper = 35),
  distance = p_dbl(lower = 0.1, upper = 50), scale = p_fct(levels = c(TRUE, FALSE))
)
search_space
hout = rsmp("custom") 
hout$instantiate(task_homo,
                 ...
                 test = list(test_set))
measure = msr("regr.mse")
measure$predict_sets <- c("test", "train")
evals20 = trm("evals", n_evals = 1e2) 
instance = TuningInstanceSingleCrit$new(
  task = task_homo,
  learner = learner,
...
trn1 = tnr("grid_search", resolution = 100, batch_size = 10)
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals
# km ===============
learner <- mlr_learners$get("regr.km")
# sink(file = "results/ML_para/km.txt")
# learner$param_set$print()
# sink()
# learner$param_set$values = list(k = 6, distance = 0.1199199)
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]]
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]]
learner$train(task_homo, row_ids = train_set)
prediction <- learner$predict(task_homo, row_ids = train_set) 
train_set_pic_km <- predplot(prediction, title = 'Kriging model: train set')
# ggsave(filename = "results/ML_vil/kmtrain_set_pic.jpeg", plot = train_set_pic_km, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
prediction <- learner$predict(task_homo, row_ids = test_set) 
test_set_pic_km <- predplot(prediction, title = 'Kriging model: test set') 
# ggsave(filename = "results/ML_vil/kmtest_set_pic.jpeg", plot = test_set_pic_km, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
# ranger !===============
learner <- mlr_learners$get("regr.ranger")
# sink(file = "results/ML_para/ranger.txt")
# learner$param_set$print()
# sink()
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]]
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]] 
learner$train(task_homo, row_ids = train_set)
prediction <- learner$predict(task_homo, row_ids = train_set) 
train_set_pic_ranger <- predplot(prediction, title = 'RF: train set')
# ggsave(filename = "results/ML_vil/rangertrain_set_pic.jpeg", plot = train_set_pic_ranger, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
prediction <- learner$predict(task_homo, row_ids = test_set) 
test_set_pic_ranger <- predplot(prediction, title = 'RF: test set')
# ggsave(filename = "results/ML_vil/rangertest_set_pic.jpeg", plot = test_set_pic_ranger, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
# RPART ===============
learner <- mlr_learners$get("regr.rpart") 
learner$param_set$values = list(minsplit = 14,
                                minbucket = 13, 
                                cp = 0.01010101, 
                                maxdepth = 20)
# sink(file = "results/ML_para/rpart.txt")
# learner$param_set$print()
# sink()
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]] 
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]]
learner$train(task_homo, row_ids = train_set)
prediction <- learner$predict(task_homo, row_ids = train_set) 
train_set_pic_RPART <- predplot(prediction, title = 'RPART: train set') 
# ggsave(filename = "results/ML_vil/RPARTtrain_set_pic.jpeg", plot = train_set_pic_RPART, width = unit(5.3,"cm"), height = unit(4.3,"cm")) 
prediction <- learner$predict(task_homo, row_ids = test_set) 
test_set_pic_RPART <- predplot(prediction, title = 'RPART: train set')
# ggsave(filename = "results/ML_vil/RPARTtest_set_pic.jpeg", plot = test_set_pic_RPART, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set 
learner$predict_sets <- c("test", "train") 
search_space = ps(
  cp = p_dbl(lower = 1e-13, upper = 1), 
  maxdepth = p_int(lower = 1, upper = 29), 
  minsplit = p_int(lower = 1, upper = 50), 
  minbucket = p_int(lower = 1, upper = 50)
)
search_space
hout = rsmp("custom") 
hout$instantiate(task_homo,
...
instance = TuningInstanceSingleCrit$new(
  ...
  learner = learner,
  resampling = hout,
  measure = measure, search_space = search_space, terminator = evals20
)
trn1 = tnr("grid_search", resolution = 100, batch_size = 10) 
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals
...
learner$param_set$values = list(nrounds = 29, max_depth = 20)
# sink(file = "results/ML_para/xgboost.txt")
# learner$param_set$print()
# sink()
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]] 
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]]
learner$train(task_homo, row_ids = train_set)
prediction <- learner$predict(task_homo, row_ids = train_set) 
train_set_pic_xgboost <- predplot(prediction, title = 'xgboost: train set') 
# ggsave(filename = "results/ML_vil/xgboosttrain_set_pic.jpeg", plot = train_set_pic_xgboost, width = unit(5.3,"cm"), height = unit(4.3,"cm")) 
prediction <- learner$predict(task_homo, row_ids = test_set)
test_set_pic_xgboost <- predplot(prediction, title = 'xgboost: test set')
# ggsave(filename = "results/ML_vil/xgboosttest_set_pic.jpeg", plot = test_set_pic_xgboost, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set 
learner$predict_sets <- c("test", "train")
search_space = ps(
  nrounds = p_int(lower = 1, upper = 29),
  max_depth = p_int(lower = 1, upper = 50) 
  )
search_space
hout = rsmp("custom") 
hout$instantiate(task_homo,
...
trn1 = tnr("grid_search", resolution = 500, batch_size = 10) 
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals
...
train_set <- c(read_csv(file = "results/temp_data/train_set.csv"))[[1]] 
test_set <- c(read_csv(file = "results/temp_data/test_set.csv"))[[1]] 
learner$train(task_lumo, row_ids = train_set)
prediction <- learner$predict(task_lumo, row_ids = train_set) 
train_set_pic_SVM <- predplot(prediction, title = 'SVM: train set', scale = 5)
# ggsave(filename = "results/ML_vil/SVMtrain_set_pic.jpeg", plot = train_set_pic_SVM, width = unit(5.3,"cm"), height = unit(4.3,"cm")) 
prediction <- learner$predict(task_lumo, row_ids = test_set)
test_set_pic_SVM <- predplot(prediction, title = 'SVM: test set', scale = 5)
# ggsave(filename = "results/ML_vil/SVMtest_set_pic.jpeg", plot = test_set_pic_SVM, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set 
learner$predict_sets <- c("test", "train")
search_space = ps(
...
trn1 = tnr("grid_search", resolution = 40, batch_size = 10) 
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals # learner$train(task)
...
learner$train(task_lumo, row_ids = train_set)
prediction <- learner$predict(task_lumo, row_ids = train_set)
train_set_pic_kknn <- predplot(prediction, title = 'knn: train set', scale = 5)
# ggsave(filename = "results/ML_vil/kknntrain_set_pic.jpeg", plot = train_set_pic_kknn, width = unit(5.3,"cm"), height = unit(4.3,"cm")) 
prediction <- learner$predict(task_lumo, row_ids = test_set)
test_set_pic_kknn <- predplot(prediction, title = 'knn: test set', scale = 5)
# ggsave(filename = "results/ML_vil/kknntest_set_pic.jpeg", plot = test_set_pic_kknn, width = unit(5.3,"cm"), height = unit(4.3,"cm"))
learner$param_set 
learner$predict_sets <- c("test", "train")
learner$predict_sets <- c("test")
search_space = ps(
  k = p_int(lower = 1, upper = 35),
  distance = p_dbl(lower = 0.1, upper = 50), 
  scale = p_fct(levels = c(TRUE, FALSE))
)
search_space
hout = rsmp("custom") 
hout$instantiate(task_homo,
...
trn1 = tnr("grid_search", resolution = 1000, batch_size = 20)
trn1$optimize(instance)
learner$param_set$values <- instance$result_learner_param_vals
